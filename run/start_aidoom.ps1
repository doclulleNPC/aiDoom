...
$wad = Join-Path $here "doom1.wad"
if (-not (Test-Path $wad)) { Die "doom1.wad not found in $here" }
if (-not (Test-Path (Join-Path $here "SDL.dll"))) { Die "SDL.dll missing next to aidoom.exe in $here" }

$gameArgs = @("-iwad","doom1.wad","-warp","$Episode","$Map","-skill","$Skill","-aidirector","$Port")
