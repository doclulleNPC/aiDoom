@echo off
REM -----------------------------------------------------------------------
REM start_aibuddy.bat -- launch BuddyDoom with the AI/LLM co-op buddy.
...
REM
REM Ollama host/port/model come from buddydoom.cfg (next to this file).  Needs a
...
if not exist "buddydoom.exe"   ( echo [aibuddy] buddydoom.exe not found -- build it first ^(see README^). & pause & exit /b 1 )
...
echo [aibuddy] game:     buddydoom.exe -aicoop -aidirector 31666 %IWAD_FLAG% %ARGS%
start "BuddyDoom" buddydoom.exe -aicoop -aidirector 31666 %IWAD_FLAG% %ARGS%

REM give the game a moment to open the listening socket
ping -n 3 127.0.0.1 >nul

echo [aibuddy] director: director.exe --port 31666  (Ollama from buddydoom.cfg)
