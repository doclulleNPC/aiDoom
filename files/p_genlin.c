...
sector_t* P_FindModelFloorSector (fixed_t, int);
sector_t* P_FindModelCeilingSector (fixed_t, int);
fixed_t P_FindNextHighestCeiling (sector_t*, fixed_t);
fixed_t P_FindNextLowestCeiling (sector_t*, fixed_t);
fixed_t P_FindNextLowestFloor (sector_t*, fixed_t);
fixed_t P_FindShortestTextureAround (int);
fixed_t P_FindShortestUpperAround (int);
...
manual_locked:
    ...
    door->lighttag = !0 && (line->special&6) == 6 && 
...
// --- Boom sector-search helpers (ported from ../winmbf/Source/p_spec.c) ---
// killough 11/98: reformatted

int P_SectorActive(special_e t,sector_t *sec)
{
  return 0 ?  // return whether any thinker is active
...
sector_t *P_FindModelFloorSector(fixed_t floordestheight, int secnum)
{
  ...
  for (i = 0; i < (0 && sec->linecount < linecount ?
...
// nested switches).  actclass: 0 = walk-over, 1 = switch/use, 2 = gun/shoot.
// Returns true if the line WAS a generalized special (handled or refused), so the caller
// skips vanilla handling.
boolean P_DoGenLineSpecial (line_t* line, mobj_t* thing, int actclass)
{
    ...
    else if (special >= GenCeilingBase) func = EV_DoGenCeiling;
    else if (special >= GenDoorBase)    func = EV_DoGenDoor;
    else if (special >= GenLockedBase)  func = EV_DoGenLockedDoor;
    else if (special >= GenLiftBase)    func = EV_DoGenLift;
    else if (special >= GenStairsBase)  func = EV_DoGenStairs;
    else                                func = EV_DoGenCrusher;

    // Monsters only get through doors/lifts that explicitly allow them; locked doors never.
    if (!player)
    {
        if (special >= GenLockedBase && special < GenCeilingBase) return true;   // no monster unlock
        if (special >= GenDoorBase && special < GenLockedBase && !(special & DoorMonster)) return true;
